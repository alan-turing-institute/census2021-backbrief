...
Were ONS's census-CCS clerical matching procedure to proceed as in 2011, there is however no guarantee that the kinds of difficult to match record pairs that an active learning system relies on are likely to arise early enough on in the matching procedure. A manual decision process may still be required for which record pairs to label first.
