...
Sarawagi and Bhamidipaty developed an active learning method to significantly reduce the size of the training set needed for de-duplication (record linkage within a single dataset), which they tested with both scientific citation and address records.  Their method starts with a small subset of record pairs that have been labeled as matches or non-matches for training a base classifier. Record pairs whose match status the classifier finds difficult to determine can be said to fall within a "confusion region", similar to the “reject region” in the *Fellegi-Sunter* algorithm. This region is initially large, but by picking the most ambiguous record pairs for clerical resolution first, those closest to the mid-point of the confusion region, the algorithm can quickly learn the peculiarities of a dataset. S&B note as measured by precision and recall values, the optimal base classifiers to use in tandem with active learning are Decision trees, which out-perform SVMs and Naive bayes. Similarly, Tejada et al. [59] developed a method using multiple Decision trees classifiers, trained using varying different data and parameters to detect consistently ambiguous record pairs to be prioritised for clerical resolution.
